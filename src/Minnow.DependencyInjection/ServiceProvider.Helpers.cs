...
namespace Minnow.DependencyInjection
{
    public partial class ServiceProvider<TServiceProvider>
    {
        ...
        public Object GetService(String name)
        {
            ...
            if(_activeServices.TryGetValue(name, out ServiceConfigurationManager<TServiceProvider> manager))
            {
                return manager.GetInstance();
            }
            
            // Check to see if this service has been registered...
            if(_registeredServices.TryGetValue(name, out ServiceConfiguration<TServiceProvider> configuration))
            {
        ...
        {
            // Check to see if this service is already active...
            if (_activeServices.TryGetValue(id, out ServiceConfigurationManager<TServiceProvider> manager))
            {
                return manager.GetInstance();
            }

            // Check to see if this service has been registered...
            if (_registeredServices.TryGetValue(id, out ServiceConfiguration<TServiceProvider> configuration))
            {
                return configuration.GetInstance(this as TServiceProvider);
            }
