...
namespace DotNetUtils.DependencyInjection.Builders
{
        ...
        where TServiceProvider : ServiceProvider<TServiceProvider>
    {
        ...
        private Dictionary<String, ServiceConfiguration<TServiceProvider>> BuildServiceConfigurations()
        {
            ...
            Dictionary<Type, TypeFactory<TServiceProvider>> allFactories = this.typeFactories.PrioritizeBy(f => f.Type)
            ...
            List<ServiceConfiguration<TServiceProvider>> allServiceConfigurations = this.serviceConfigurations.PrioritizeBy(s => s.Name)
                .Select(s => s.Build(allFactories, allSetups))
                .ToList();

            return allServiceConfigurations.ToDictionaryByValue(sc => sc.Name);
        }
        ...
        {
            return this.Build(
                services: this.BuildServiceConfigurations());
        }
        #endregion

        #region Abstract Methods
        protected abstract TServiceProvider Build(Dictionary<String, ServiceConfiguration<TServiceProvider>> services);
        #endregion
    }
}
