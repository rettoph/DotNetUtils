...
{
    public partial class ServiceProvider<TServiceProvider>
    {
        ...
        public Object GetService(String name)
        {
            ...
            if(_activeNamesServices.TryGetValue(name, out ServiceConfigurationManager<TServiceProvider> manager))
            {
                return manager.GetInstance();
            }
            
            // Check to see if this service has been registered...
            if(_registeredNamesServices.TryGetValue(name, out ServiceConfiguration<TServiceProvider> configuration))
            {
                return configuration.GetInstance(this as TServiceProvider);
            }
        ...
        {
            // Check to see if this service is already active...
            if (_activeIdsServices.TryGetValue(id, out ServiceConfigurationManager<TServiceProvider> manager))
            {
                return manager.GetInstance();
            }

            // Check to see if this service has been registered...
            if (_registeredIdsServices.TryGetValue(id, out ServiceConfiguration<TServiceProvider> configuration))
            {
                return configuration.GetInstance(this as TServiceProvider);
            }
            ...
            Int32 customSetupOrder = 0)
        {
            ...
            if (_registeredNamesServices.TryGetValue(name, out ServiceConfiguration<TServiceProvider> configuration))
            {
                return configuration.GetInstance(this as TServiceProvider, customSetup, customSetupOrder);
            }
        ...
        public T GetService<T>(
            String name,
            Action<T, TServiceProvider, ServiceConfiguration<TServiceProvider>> customSetup,
            Int32 customSetupOrder = 0)
                where T : class
